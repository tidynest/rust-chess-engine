...
use chess_core::{Color, GameState, Move, Piece, Square};
use chess::{ChessMove, Color as ChessColor, File, Piece as ChessPiece, Rank, Square as ChessSquare};
use eframe::egui::{self, Pos2, Rect, Response, Ui, Vec2, CornerRadius, Color32};

use crate::app::state::ChessApp;
use crate::utils::conversions::{convert_to_chess_piece, convert_piece_type};
...
impl ChessApp {
    ...
    pub fn draw_board(&mut self, ui: &mut Ui) -> Response {
        ...
        for rank in 0..8 {
            for file in 0..8 {
                ...
                self.draw_square(square, square_rect, rank, file, display_rank, display_file, square_size, &painter);
                self.draw_square_labels(rank, file, display_rank, display_file, square_rect, &painter);
    ...
    fn handle_board_interactions(&mut self, response: &Response, board_rect: Rect, square_size: f32) {
        ...
        // Handle click
        if response.clicked() {
            if let Some(square) = crate::utils::coords::get_square_from_pos(
        ...
        if response.drag_stopped() {
            if let Some((from_square, _, _)) = self.dragging_piece {
                if let Some(to_square) = crate::utils::coords::get_square_from_pos(
